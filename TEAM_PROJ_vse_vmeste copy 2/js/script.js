...
//         // footer.mediaContent = user.link;
//         userCardContainer.append(card);
        
//         // return { name: user.name, email: user.email, link:user.link,element: card};
//         return { name: user.name, element: card};
//     });
// });
...
var wrapper = document.getElementById("SEARCH-Wrapper-ID");
var userCards = document.getElementById("SEARCH-user-c");

// Добавляем слушатель события клика на всю страницу
document.addEventListener("click", function (event) {
  // Проверяем, является ли цель клика элементом внутри "SEARCH-Wrapper-ID"
  var isClickedInsideWrapper = wrapper.contains(event.target);

  // Если клик был совершен вне "SEARCH-Wrapper-ID", скрываем "SEARCH-user-c"
  if (!isClickedInsideWrapper) {
    userCards.style.display = "none";
  }
});


// Добавляем слушатель события наведения курсора на "SEARCH-Wrapper-ID"
wrapper.addEventListener("mouseenter", function () {
  // Показываем "SEARCH-user-c" при наведении курсора
  userCards.style.display = "flex";
});

// Добавляем слушатель события ухода курсора из "SEARCH-Wrapper-ID"
wrapper.addEventListener("mouseleave", function () {
  // Скрываем "SEARCH-user-c" при уходе курсора
  userCards.style.display = "none";
});

// document.getElementById("SEARCH-search").addEventListener("focus", function () {
//     document.getElementById("SEARCH-user-c").style.display = "flex";
// });

// document.getElementById("SEARCH-search").addEventListener("blur", function () {
//     document.getElementById("SEARCH-user-c").style.display = "none";
// });
...
//         // const link = card.querySelector("[data-link]");
//         // link.href = user.link || "#"; // Если у пользователя есть ссылка, присвоить ее в атрибут href, иначе присвоить "#" (или другую ссылку по умолчанию)
...
//         // return { name: user.name, email: user.email, link:user.link,element: card};
//         // return { name: user.name, link: user.link, element: card};
//         return { name: user.name, element: card};
//     });
// });
...
.then((data) => {
    users = data.map((user) => {
        ...
        // Добавляем слушатель события клика на элемент SEARCH-poisk-id
        card.querySelector("#SEARCH-poisk-id").addEventListener("click", function () {
            if (user.link) {
                window.location.href = user.link; // Переход по ссылке из JSON
            }
        });
